...
void SrcSnkDDA::analyze2(SVFModule module) {
    ...
    for (SVFGNodeSetIter iter = sourcesBegin(), eiter = sourcesEnd();
        ...
        for(SVFGNodeSetIter sit = getCurSlice()->sinksBegin(), esit =getCurSlice()->sinksEnd(); sit != esit; ++sit){
            SVFGNode* node=getNode(getlastNode((*sit)->getId()));
            for(SVFGNodeSetIter it=getCurSlice()->forwardSliceBegin(), eit =
                        getCurSlice()->forwardSliceEnd(); it!=eit; ++it){
                if((*it)->getNodeKind()==2){
                    continue;
                }
                bool flag=getCurSlice()->isUseAfterFree(node,*it);
                if(!flag){
                    reportBug(getCurSlice());
                }
            }
